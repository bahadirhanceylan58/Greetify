...
const CATEGORIES: { id: InvitationCategory | 'all'; label: string; emoji: string }[] = [
  { id: 'all',          label: 'Tümü',      emoji: '✨' },
  { id: 'wedding',      label: 'Düğün',     emoji: '💍' },
  { id: 'engagement',   label: 'Nişan',     emoji: '💑' },
  { id: 'birthday',     label: 'Doğum Günü',emoji: '🎂' },
  { id: 'circumcision', label: 'Sünnet',    emoji: '🌙' },
  { id: 'henna',        label: 'Kına',      emoji: '🌸' },
  { id: 'graduation',   label: 'Mezuniyet', emoji: '🎓' },
  { id: 'baby',         label: 'Bebek',     emoji: '👶' },
  { id: 'opening',      label: 'Diğer',     emoji: '🎊' },
]
...
export default function InvitePage() {
  ...
  const filtered = useMemo(() =>
    active === 'all' ? invitationTemplates : invitationTemplates.filter(t => t.category === active),
    [active]
  )
    ...
    <main className="min-h-screen">
      ...
      <header className="bg-gradient-to-b from-gray-900 to-transparent pt-10 pb-6 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <Link href="/" className="inline-block text-gray-500 hover:text-gray-300 text-sm mb-4 transition-colors">← Tebrik Kartları</Link>
          ...
          <p className="text-gray-400 text-sm max-w-md mx-auto">
            Düğün, nişan, doğum günü ve daha fazlası için kişiselleştirilmiş davetiye kartları
          </p>
        </div>
      </header>
              ...
              <div className="group cursor-pointer rounded-2xl overflow-hidden transition-all duration-300 hover:scale-105 hover:-translate-y-1"
                ...
                <div className="relative h-52 overflow-hidden">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center z-20 opacity-0 group-hover:opacity-100 transition-opacity bg-black/20">
                    <span className="bg-white text-gray-900 font-bold px-4 py-2 rounded-full text-sm shadow-2xl">+ Oluştur</span>
                  </div>
                </div>
                {/* Info */}
