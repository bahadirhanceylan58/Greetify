...
export default function InvitationEditor({ template }: Props) {
  ...
  }, [template, hostName, eventDate, eventTime, venue, address, message, fontFamily, accentColor, overlayOpacity, cardSize, bgStyle, customBg])
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      ...
      <nav className="flex items-center gap-3 px-4 py-4 border-b border-gray-800">
        <Link href="/invite" className="text-gray-400 hover:text-white transition-colors">← Geri</Link>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1">
          <h2 className="text-gray-400 text-sm font-medium mb-3 uppercase tracking-wide">Önizleme</h2>
          <div className={`relative rounded-2xl overflow-hidden bg-gray-900 shadow-2xl ${cardSize === '9:16' ? 'aspect-[9/16]' : 'aspect-square'}`}>
            {previewUrl ? (
              ...
              <img src={previewUrl} alt="Davetiye önizleme"
          ...
          <div className="mt-4 flex gap-3">
            <button onClick={handleDownload} disabled={isGenerating}
              ...
              {downloaded ? '✓ İndirildi!' : '⬇ PNG İndir'}
            </button>
        ...
        <div className="lg:w-80 space-y-4">
          <h2 className="text-gray-400 text-sm font-medium uppercase tracking-wide">Özelleştir</h2>

          {/* Event Info */}
          <div className="bg-gray-900 rounded-2xl p-5 space-y-4">
            <p className="text-white font-medium text-sm">Etkinlik Bilgileri</p>

            <div>
              <label className="text-gray-400 text-xs mb-1 block">İsim / Çift İsmi</label>
              <input type="text" placeholder="Ahmet & Fatma" value={hostName}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-gray-400 text-xs mb-1 block">Tarih</label>
              ...
              <div>
                <label className="text-gray-400 text-xs mb-1 block">Saat</label>
            ...
            <div>
              <label className="text-gray-400 text-xs mb-1 block">Mekan</label>
              <input type="text" placeholder="Hilton İstanbul" value={venue}
                onChange={(e) => setVenue(e.target.value)} maxLength={60}
                className="w-full bg-gray-800 border border-gray-700 focus:border-amber-500 rounded-xl px-4 py-2.5 text-white placeholder-gray-500 outline-none text-sm" />
            </div>

            <div>
              <label className="text-gray-400 text-xs mb-1 block">Adres <span className="text-gray-600">(isteğe bağlı)</span></label>
              <input type="text" placeholder="Harbiye, Şişli, İstanbul" value={address}
                onChange={(e) => setAddress(e.target.value)} maxLength={80}
                className="w-full bg-gray-800 border border-gray-700 focus:border-amber-500 rounded-xl px-4 py-2.5 text-white placeholder-gray-500 outline-none text-sm" />
            </div>

            <div>
              <label className="text-gray-400 text-xs mb-1 block">Not / Mesaj <span className="text-gray-600">(isteğe bağlı)</span></label>
              <textarea placeholder="Lütfen RSVP yapınız..." value={message}
          ...
          {/* Design */}
          <div className="bg-gray-900 rounded-2xl p-5 space-y-4">
            <p className="text-white font-medium text-sm">Tasarım</p>

            {/* Accent color */}
            <div>
              <label className="text-gray-400 text-xs mb-2 block">Vurgu Rengi</label>
            ...
            {/* Font */}
            <div>
              <label className="text-gray-400 text-xs mb-2 block">Yazı Tipi</label>
              <div className="grid grid-cols-2 gap-2">
            ...
            {/* Card size */}
            <div>
              <label className="text-gray-400 text-xs mb-2 block">Kart Boyutu</label>
            ...
            {/* Background */}
            <div>
              <label className="text-gray-400 text-xs mb-2 block">Arka Plan</label>
              ...
              {bgStyle === 'custom' && (
                customBg ? (
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => setCustomBg(null)} className="text-xs text-red-400">✕ Kaldır</button>
                  </div>
                ) : (
              ...
              {bgStyle !== 'gradient' && (
                <div className="mt-2">
                  <label className="text-gray-400 text-xs mb-1 block">Karartma: <span className="text-amber-400">{overlayOpacity}%</span></label>
